...
# -*- coding: utf-8 -*-
"""
seekur_interactive_tcp.py — Contrôleur interactif SeekurJR (TCP simulation)

Version pour la SIMULATION. Se connecte via TCP au simulateur du protocole
SeekurOS (seekur_protocol_simulator, port 9999 par défaut).

Contient un SerialTCPAdapter qui bascule automatiquement entre TCP (préfixe
tcp:// dans --port) et port série classique. En pratique, utilisé
exclusivement avec tcp://localhost:9999 pour tester le simulateur ou un
launch de simulation.

Pour le vrai robot en série, utiliser seekur_interactive_serial.py.
"""
